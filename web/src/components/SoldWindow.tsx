...
/**
 * The copies that left the shelf, with what they made. The gain here is
 * realized, money that changed hands, which is why it sits apart from the
 * shelf value's paper gain.
 */
export function SoldWindow({ summary, byId }: { summary: SoldSummary; byId: Map<string, CatalogGame> }) {
  ...
  return (
    <section aria-label="Sold">
      <Window title="Sold" bodyClassName="p-0" order={2}>
        <div className="flex flex-wrap items-end gap-x-8 gap-y-3 border-b-2 border-[var(--border)] p-3">
          {summary.compared > 0 ? (
            <>
              <div>
                <p className="eyebrow">Paid</p>
                <p className="tabular text-lg font-bold">{moneyExact(summary.paid_cents)}</p>
              </div>
              <div>
                <p className="eyebrow">Sold for</p>
                <p className="tabular text-lg font-bold">{moneyExact(summary.sold_cents)}</p>
              </div>
              <div>
                <p className="eyebrow">Realized gain</p>
                <p className="flex items-center gap-2">
                  <span className="tabular text-lg font-bold" style={{ color: heat(summary.gain_pct) }}>
              ...
              {summary.lines.map((l) => {
                ...
                return (
                  <tr key={l.item.id ?? id} className="border-b border-[var(--input)] last:border-0 hover:bg-[var(--secondary)]">
                    ...
                    <td className="tabular p-2 text-right text-xs font-bold">
                      {l.sold_cents === null ? '—' : moneyExact(l.sold_cents)}
                    </td>
